...
export default function Navigation() {
  ...
  useEffect(() => {
    async function loadNavigation() {
      try {
        // Set up timeout for Sanity data loading (5 seconds)
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(
            () => reject(new Error("Navigation loading timeout")),
            5000,
          );
        });
        ...
        const [navigationMenu, settings] = (await Promise.race([
          dataPromise,
          timeoutPromise,
        ])) as [NavigationMenu, SiteSettings];
  ...
  // Don't show loading skeleton - show navigation immediately instead
  // const loading = false; // Always show navigation immediately (removed to fix variable conflict)
                      ...
                      {item.megaMenu.map((section: any) => (
                        <div key={section.sectionTitle} className="space-y-1">
                          ...
                          <div className="grid grid-cols-2 gap-1">
                            {section.links
                              .filter((link: any) => link.isVisible !== false)
                              .map((link: any) => (
                                <Link
                                  key={link.name}
                                  href={link.href}
