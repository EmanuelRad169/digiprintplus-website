                ...
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-gray-600">
                      {spec.value} {spec.unit && <span className="text-sm">{spec.unit}</span>}
                    </span>
                  </div>
                </div>
              ))}
        ...
        <nav className="flex space-x-4 px-4" aria-label="Tabs">
          {tabs.map((tab) => (
            <button
              ...
              className={`py-2 px-3 border-b-2 font-medium text-sm transition-colors rounded-t-lg ${
                activeTab === tab.id
                  ? 'border-magenta-500 text-magenta-600 bg-gray-50'
