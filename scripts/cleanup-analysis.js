...
const EXCLUDED_DIRS = ["node_modules", ".next", "dist", ".git", ".turbo"];
...
class CleanupAnalyzer {
  ...
  analyzeFile(fullPath, relativePath) {
    ...
    if (
      fileName.match(/(test|spec|example|temp|debug|old|backup|unused|\.bak)/i)
    ) {
  ...
  generateReport() {
    ...
    if (this.debugFiles.length > 0) {
      report.recommendations.push(
        "Review and remove debug/test files from production code",
      );
    }
    if (this.duplicates.length > 0) {
      report.recommendations.push(
        "Consolidate or remove duplicate/versioned files",
      );
    }
    if (unimported.length > 10) {
      report.recommendations.push(
        `${unimported.length} files appear unused - verify and clean up`,
      );
    }
