...
async function phase1_validateEnvironment() {
  ...
  if (fs.existsSync(configPath)) {
    ...
    if (configContent.includes("output:")) {
      const isStatic = configContent.includes(
        'output: process.env.NETLIFY ? "export"',
      );
      logResult({
        phase: "NEXT_CONFIG",
        status: "pass",
...
async function phase3_validateRoutes() {
  ...
  try {
    ...
    const products =
      await client.fetch<Array<{ slug: string; title: string }>>(productsQuery);
    ...
    if (products.length > 0) {
      logResult({
        ...
        details: {
          samples: products.slice(0, 3).map((p) => `/products/${p.slug}`),
        },
      });
    } else {
      logResult({
    ...
    const blogs =
      await client.fetch<Array<{ slug: string; title: string }>>(blogQuery);
    ...
    const routeFiles = [
      {
        path: "apps/web/src/app/products/[slug]/page.tsx",
        route: "/products/[slug]",
      },
    ...
    for (const { path: filePath, route } of routeFiles) {
      ...
      if (fs.existsSync(fullPath)) {
        ...
        const hasGenerateStaticParams = content.includes(
      ...
      if (filePath.includes("robots.ts")) {
        ...
        if (content.includes("marvelous-treacle-ca0286.netlify.app")) {
          logResult({
            ...
            status: "warn",
            message:
              "robots.ts contains hardcoded Netlify URL - should use env variable",
          });
        }
      }
      ...
      if (filePath.includes("sitemap.ts")) {
        ...
        if (content.includes("marvelous-treacle-ca0286.netlify.app")) {
          logResult({
            ...
            status: "warn",
            message:
              "sitemap.ts contains hardcoded URL - should use NEXT_PUBLIC_SITE_URL",
          });
        }
      }
    } else {
