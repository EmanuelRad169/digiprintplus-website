...
const staticFound = results.filter(
  (r) => r.type === "static" && r.exists,
).length;
const seoFound = results.filter((r) => r.type === "seo" && r.exists).length;
const dynamicFound = results.filter(
  (r) => r.type === "dynamic" && r.exists,
).length;

console.log(`Static Pages: ${staticFound} found`);
console.log(`SEO Files: ${seoFound}/2 found`);
console.log(
  `Dynamic Pages: ${dynamicFound > 0 ? "✅ Generated" : "❌ Not generated"}`,
);

const criticalMissing = results.filter(
  (r) => (r.type === "seo" || r.file === "index.html") && !r.exists,
);
