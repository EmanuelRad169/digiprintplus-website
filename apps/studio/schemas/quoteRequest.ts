...
export default defineType({
  ...
  preview: {
    ...
    prepare(selection: any) {
      const { requestId, firstName, lastName, company, productType, status, priority, amount, currency } = selection
      
      // Create customer display name
      const customerName = `${firstName || ''} ${lastName || ''}`.trim()
      const displayName = company ? `${customerName} (${company})` : customerName
      ...
      if (priority !== 'normal') parts.push(`${priorityEmojis[priority] || ''} ${priority}`)
