...
// Wrapper to track failures locally
  ...
  try {
    ...
    if (response.status === 200 && data.message?.includes("active")) {
      ...
      if (data.revalidateEndpoint || data.instantRevalidation) {
        ...
        logWarning(
          `Instant revalidation not configured (will use full rebuild)`,
        );
      }
      ...
      logErrorWithTracking(`Webhook endpoint returned unexpected response`);
      checks.push({
        name: "Webhook Endpoint",
        passed: false,
...
async function checkRevalidateSecurity() {
  ...
  if (SANITY_REVALIDATE_SECRET) {
    ...
    logInfo(
      "Set SANITY_REVALIDATE_SECRET env var to test authenticated access",
    );
  }
  ...
  if (failed === 0) {
    ...
    if (failedChecks.length > 0) {
      ...
      failedChecks.forEach((check) => {
        console.log(
          `  ${colors.red}→${colors.reset} ${check.name}: ${check.message}`,
        );
      });
    }
  }
...
async function main() {
  console.log(
    `${colors.bold}🔐 Webhook Health & Security Verification${colors.reset}`,
  );
  console.log("=".repeat(60));
  console.log(`Site URL: ${LIVE_SITE_URL}`);
  console.log(
    `Secret configured: ${SANITY_REVALIDATE_SECRET ? "Yes ✅" : "No ⚠️"}`,
  );
