...
function verifyPnpmVersion() {
  ...
  if (majorVersion === expectedMajor) {
    ...
    logWarning(
      `pnpm ${currentVersion} (expected ${expectedMajor}.x) - may work but not guaranteed`,
    );
  }
}
...
function verifyNextVersion() {
  ...
  if (
    ...
    currentVersion.startsWith("15.5.")
  ) {
    logSuccess(
      `Next.js ${currentVersion} (matches expected ${expectedVersion})`,
    );
  } else {
    logError(
      `Next.js ${currentVersion} does NOT match expected ${expectedVersion}`,
    );
...
function verifyEslintVersion() {
  ...
  if (!eslintVersion) {
    logWarning(
      "ESLint not installed (lint may be disabled in production builds)",
    );
    return;
  }
  ...
  if (eslintMajor === 8) {
    ...
    logError(
      `ESLint ${eslintVersion} (expected 8.x) - Next.js 15 requires ESLint 8!`,
    );
    hasErrors = true;
  }
  ...
  if (eslintConfigNextVersion && nextVersion) {
    ...
    const configMajorMinor = eslintConfigNextVersion
      .split(".")
      .slice(0, 2)
      .join(".");
    ...
    if (nextMajorMinor === configMajorMinor) {
      logSuccess(
        `eslint-config-next ${eslintConfigNextVersion} matches Next.js ${nextVersion}`,
      );
    } else {
      logError(
        `eslint-config-next ${eslintConfigNextVersion} does NOT match Next.js ${nextVersion}`,
      );
      logError(`Expected eslint-config-next ${nextMajorMinor}.x`);
      logError(
        'This will cause "Cannot find module babel/eslint-parser" errors!',
      );
      hasErrors = true;
    }
  }
...
function verifyWorkspaceConsistency() {
  ...
  try {
    ...
    const rootNextVersion =
      rootPackageJson.devDependencies?.next ||
      rootPackageJson.dependencies?.next;
    const webNextVersion =
      webPackageJson.devDependencies?.next || webPackageJson.dependencies?.next;
    ...
    if (rootNextVersion && webNextVersion) {
      ...
      if (
        normalizeVersion(rootNextVersion) === normalizeVersion(webNextVersion)
      ) {
        logSuccess(
          `Next.js versions aligned: ${rootNextVersion} = ${webNextVersion}`,
        );
      } else {
        logWarning(
          `Next.js version mismatch: root=${rootNextVersion}, web=${webNextVersion}`,
        );
    ...
    if (rootPackageJson.pnpm?.overrides) {
      ...
      logWarning(
        "No pnpm overrides found - consider adding them to lock versions",
      );
    }
  } catch (error) {
...
function main() {
  ...
  if (hasErrors) {
    ...
    console.log(`\nQuick fix:`);
    console.log(`  1. nvm use 18 (or nvm install 18)`);
