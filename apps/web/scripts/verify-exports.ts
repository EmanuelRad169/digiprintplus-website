...
import { colors, logSuccess, logError, logHeader } from './utils/verification-utils';
...
async function verifyFetchersExports() {
  ...
  try {
    ...
    logError(`Failed to import fetchers module: ${error instanceof Error ? error.message : 'Unknown error'}`);
    hasErrors = true;
  }
}
