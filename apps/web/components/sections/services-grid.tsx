...
export function ServicesGrid({ featuredOnly = false, limit, showCTA = true }: ServicesGridProps) {
  ...
  useEffect(() => {
    async function loadServices() {
      try {
        ...
        let servicesToShow = sanityServices
        
        if (limit && servicesToShow.length > limit) {
          servicesToShow = servicesToShow.slice(0, limit)
        }
  ...
  return (
    <section className="py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        ...
        <ServicesGrid featuredOnly={true} limit={6} />
      </div>
    </section>
  )
}
