...
export async function getFeaturedBlogPosts(limit: number = 3, isPreview = false): Promise<BlogPost[]> {
  try {
    const query = isPreview
...
export async function getBlogCategories(isPreview = false): Promise<BlogCategory[]> {
  try {
    const query = isPreview
