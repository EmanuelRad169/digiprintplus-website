/**
 * Footer shape and default content — no Sanity client, no network.
 *
 * Split out of `footer.ts` so client components can import the type and the
 * fallback without pulling `@sanity/client` into the browser bundle. The
 * footer renders in the root layout, so that import cost applied to every
 * page on the site.
 */
