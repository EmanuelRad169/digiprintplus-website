...
export function generateProductSEO({
  ...
  const description =
    ...
    product.description ||
    `Professional ${product.title.toLowerCase()} printing services. Get a custom quote today.`;
  const keywords =
    product.seo?.keywords ||
    product.tags ||
    ([product.title, "printing", "custom printing", category].filter(
      Boolean,
    ) as string[]);
...
export function generateCategorySEO({
  ...
  const title =
    category.seo?.metaTitle || `${category.title} Printing Services`;
  const description =
