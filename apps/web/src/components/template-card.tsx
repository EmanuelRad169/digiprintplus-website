  ...
  return (
    ...
    >
      <div className="relative">
        <div
          className={`aspect-video overflow-hidden ${compact ? "aspect-[4/3]" : ""}`}
        >
      ...
      <div className={`p-${compact ? "3" : "4"}`}>
        <h3
          className={`font-bold text-gray-900 mb-2 group-hover:text-magenta-600 transition-colors ${
            compact ? "text-sm" : ""
          }`}
        >
          {template.title}
        ...
        >
          <span>
            {template.fileType} • {template.size}
          </span>
          ...
          <div className="flex flex-wrap gap-1 mb-4">
            ...
            {template.tags.length > (compact ? 2 : 3) && (
              <span className="text-xs text-gray-400">
                +{template.tags.length - (compact ? 2 : 3)} more
              </span>
            )}
          </div>
        )}
        ...
        >
          {template.downloadCount} download
